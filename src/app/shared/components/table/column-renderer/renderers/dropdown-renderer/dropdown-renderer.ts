...
@Component({
  ...
  template: `
    <div class="field p-float-label">
      <p-select [disabled]="data[col.renderer_option?.disabled_field]" appendTo="body" [filter]="false"
                [showClear]="false" (onChange)="change(setting, col, $event.value)" [autoDisplayFirst]="false"
                [options]="data[col.renderer_option?.list_field] | async" [ngModel]="data[col.field] || 'idle'"
                [optionLabel]="col.renderer_option?.optionLabel || 'name'"
                [optionValue]="col.renderer_option?.optionValue || 'id'"></p-select>
