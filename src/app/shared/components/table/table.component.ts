...
export class TableComponent extends Utility implements OnInit, OnDestroy {
    ...
    this._route.queryParams.pipe(takeUntil(this.destroy$), distinctUntilChanged()).subscribe(q => {
      if (q?.page && q?.limit) {
        if (this._localStorage.getItem('active-table-pagination')?.tableName == this.settingTable$.value.setting.tableName) {
          this.countFirstRow = (parseInt(q?.page) - 1) * parseInt(q?.limit)
          this.limit = parseInt(q?.limit)
        }
      }
      this.filterGlobal = q?.search || ''
    })
  ...
  paginate(event: IPaginate) {
    ...
    this._router.navigate([], { queryParams: { ...this._route.snapshot.queryParams, page: event?.page + 1, limit: event?.rows }, relativeTo: this._route, queryParamsHandling: 'merge' }).then(() => {
  ...
  onFilters(e) {
    e.filters = { ...e.filters, key: [{ value: e.filters?.['global']?.value || this.filterGlobal || '' }] }
    ...
    this.filter?.filters?.key ? (queryParams.search = this.filter?.filters?.key) : delete queryParams?.search
    ...
    if (savedColumns) {
      let columns = this.settingTable$.value.columns
        .filter(column => {
          ...
          return this._permissionService.checkAccess({ default: column.permissions })
        })
        .map(column => {
