...
import { CommonModule, DOCUMENT, NgClass, NgIf, NgStyle } from '@angular/common'
import { ChangeDetectorRef, Component, ContentChildren, ElementRef, EventEmitter, forwardRef, Inject, Input, NgModule, NgZone, OnDestroy, OnInit, Output, QueryList, Renderer2, TemplateRef, ViewChild, ViewEncapsulation, ChangeDetectionStrategy } from '@angular/core'
...
@Component({
  ...
  template: `
    <p-calendar styleClass="hidden"></p-calendar>
    <span
      #container
      [ngClass]="{
      ...
      [class]="styleClass"
    >
      <ng-template #inputContainer [ngIf]="!inline">
        <input
          pInputText
          #inputfield
          type="text"
          ...
          [ngClass]="'p-datepicker-input p-component'"
          autocomplete="off"
        ...
        <!--            [styleClass]="'p-calendar-clear-icon'"-->
        <!--            (click)="clear()"-->
        <!--          />-->
        <!--          <span-->
        <!--            *ngIf="clearIconTemplate"-->
        <!--            class="p-calendar-clear-icon"-->
        <!--            (click)="clear()"-->
        <!--          >-->
        <!--            <ng-template *ngTemplateOutlet="clearIconTemplate"></ng-template>-->
        <!--          </span>-->
        <!--        </ng-container>-->
        <button *ngIf="showClear && !disabled && value != null" type="button" [attr.aria-label]="iconAriaLabel" pButton
                pRipple (click)="clear()" class="p-datepicker-cleaner p-button-icon-only" [disabled]="disabled"
                tabindex="0">
          <span class="pi pi-times"></span>
        </button>
        <button type="button" [attr.aria-label]="iconAriaLabel" pRipple *ngIf="showIcon" (click)="showOverlay()"
                class="p-datepicker-dropdown" [disabled]="disabled" tabindex="0">
          <span *ngIf="icon" [ngClass]="icon"></span>
          <ng-container *ngIf="!icon">
            <CalendarIcon *ngIf="!triggerIconTemplate && !timeOnly" />
            <ng-template *ngTemplateOutlet="triggerIconTemplate"></ng-template>
          </ng-container>
        </button>
        <p-popover #jumpDayOverlay [style]="{ 'margin-top': 'unset' }" [dismissable]="true">
          <div>
            <button (click)="jumpDaySubmit(0)" class="btn btn-outline-secondary width-available"
                    translate="feedback.today"></button>
          </div>
          <div>
            <span>
              <button (click)="jumpDaySubmit(-1)" class="btn btn-outline-secondary"
                      translate="feedback.previous_day"></button>
            </span>
            <span>
              <button (click)="jumpDaySubmit(1)" class="btn btn-outline-secondary"
                      translate="feedback.next_day"></button>
            </span>
          </div>
          <div>
            <span>
              <button (click)="jumpDaySubmit(-7)" class="btn btn-outline-secondary"
                      translate="feedback.previous_week"></button>
            </span>
            <span>
              <button (click)="jumpDaySubmit(7)" class="btn btn-outline-secondary"
                      translate="feedback.next_week"></button>
            </span>
          </div>
          <div>
            <span>
              <button (click)="jumpDaySubmit(-30)" class="btn btn-outline-secondary"
                      translate="feedback.previous_month"></button>
            </span>
            <span>
              <button (click)="jumpDaySubmit(30)" class="btn btn-outline-secondary"
                      translate="feedback.next_month"></button>
            </span>
          </div>
        </p-popover>
      </ng-template>
      <div
        style="position: absolute;margin-top: calc(-2px) !important;"
        #contentWrapper
      ...
      >
        ...
        <ng-container *ngIf="!timeOnly">
          <div class="p-datepicker-calendar-container">
            <div class="p-datepicker-calendar" *ngFor="let month of months; let i = index">
              <div class="p-datepicker-header" dir="ltr">
                <button (keydown)="onContainerButtonKeydown($event)" class="p-button p-component p-button-rounded p-button-text p-button-sm p-datepicker-prev-button p-button-icon-only p-link"
                        (click)="onPrevButtonClick($event)" *ngIf="i === 0" type="button" pRipple>
                ...
                <div class="p-datepicker-title">
                  <button type="button" (click)="switchToMonthView($event)" (keydown)="onContainerButtonKeydown($event)"
                          *ngIf="currentView === 'date'" class="p-datepicker-select-month p-link"
                          [disabled]="switchViewButtonDisabled()">
                    {{ jMonthName(month.month) }}
                  </button>
                  <button type="button" (click)="switchToYearView($event)" (keydown)="onContainerButtonKeydown($event)"
                          *ngIf="currentView !== 'year'" class="p-datepicker-select-year p-link"
                          [disabled]="switchViewButtonDisabled()">
                    {{ getYear(month) }}
                  </button>
                ...
                <button (keydown)="onContainerButtonKeydown($event)" class="p-button p-component p-button-rounded p-button-text p-button-sm p-datepicker-next-button p-button-icon-only"
                ...
                <table class="p-datepicker-calendar">
                  ...
                  <tbody>
                    <tr *ngFor="let week of month.dates; let j = index">
                      ...
                      <td class="p-datepicker-day-cell"
          ...
          <div class="p-datepicker-year-view" *ngIf="currentView === 'year'">
            <span *ngFor="let y of yearPickerValues()" (click)="onYearSelect($event, y)"
                  (keydown)="onYearCellKeydown($event, y)" class="p-datepicker-year"
                  [ngClass]="{ 'p-highlight': isYearSelected(y) }" pRipple>
              {{ y }}
            </span>
          </div>
        </ng-container>
        <div class="p-datepicker-time-picker" dir="ltr" *ngIf="(showTime || timeOnly) && currentView === 'date'">
          <div class="p-datepicker-hour-picker">
            <button class="p-button p-button-rounded p-button-text p-button-sm p-datepicker-increment-button p-button-icon-only p-link" type="button" (keydown)="onContainerButtonKeydown($event)"
                    (keydown.enter)="incrementHour($event)" (keydown.space)="incrementHour($event)"
                    (mousedown)="onTimePickerElementMouseDown($event, 0, 1)"
                    (mouseup)="onTimePickerElementMouseUp($event)" (keyup.enter)="onTimePickerElementMouseUp($event)"
                    (keyup.space)="onTimePickerElementMouseUp($event)" (mouseleave)="onTimePickerElementMouseLeave()"
                    pRipple>
        ...
        <div class="p-datepicker-buttonbar" *ngIf="showButtonBar">
          <button type="button" [label]="locale.today" (keydown)="onContainerButtonKeydown($event)"
                  (click)="onTodayButtonClick($event)" pButton pRipple [ngClass]="[todayButtonStyleClass]"></button>
          <button type="button" [label]="locale.clear" (keydown)="onContainerButtonKeydown($event)"
                  (click)="onClearButtonClick($event)" pButton pRipple [ngClass]="[clearButtonStyleClass]"></button>
        </div>
...
export class PrimeCalendar implements OnInit, OnDestroy, ControlValueAccessor {
  ...
  set minDate(date: moment.Moment) {
    ...
    if (this._minDate > moment(this.value, this.format)) {
      if (this.minDateFeedback) {
        ...
        this._notify.error({message: this._translate.instant(parts[0], {date1: this._translate.instant(parts[1]), date2: this._translate.instant(parts[2])})})
      }
      this.clear()
    }
  }
  ...
  getWeekNumber(date: moment.Moment) {
    ...
    return (
      Math.floor(
        Math.round((time - checkDate.unix()) / 86400000) / 7
      ) + 1
    )
  }
  ...
  updateModel(value: any) {
    if (this.format) {
      this.value = value ? moment(value, this.format)
        .locale('en')
        .format(this.format) : null
      this.format.toLowerCase() == 'x' ? this.value = +this.value : null
    } else {
      this.value = value
    }
  ...
  isMonthSelected(month: number) {
    if (this.isComparable() && !this.isMultipleSelection()) {
      const [start, end] = this.isRangeSelection()
        ? this.value
  ...
  isMonthDisabled(month: number) {
    for (
      let day = 1;
      day < this.getDaysCountInMonth(month, this.currentYear) + 1;
      day++
    ) {
  ...
  isDateEquals(value: any, dateMeta: any) {
    ...
    if (value && ObjectUtils.isDate(value.toDate()))
      return (
  ...
  isDateDisabled(day: number, month: number, year: number): boolean {
    if (this.disabledDates) {
      for (let disabledDate of this.disabledDates) {
        if (
          disabledDate.jYear() === year &&
          disabledDate.jMonth() === month &&
  ...
  validateTime(
    ...
    if (
      this.minDate &&
      valueDateString &&
      this.minDate.format() === valueDateString
    ) {
    ...
    if (
      this.maxDate &&
      valueDateString &&
      this.maxDate.format() === valueDateString
    ) {
    ...
    if (this.isSingleSelection() && !this.timeOnly) {
      if (this._showTime) {
        ...
        val = moment
          .from(parts?.[0], 'fa')
          .locale('en')
          .format(this.getDateFormatForMoment())
        val = `${val} ${parts?.[1]}`
      } else {
        val = moment
          .from(val, 'fa')
      ...
      if (this.appendTo) {
        if (this.view === 'date') {
          this.overlay.style.width =
            DomHandler.getOuterWidth(this.overlay) + 'px'
  ...
  enableModality(element: any) {
    if (!this.mask && this.touchUI) {
      ...
      this.renderer.setStyle(
        this.mask,
        'zIndex',
        String(parseInt(element.style.zIndex) - 1)
      )
      let maskStyleClass =
  ...
  writeValue(value: any): void {
    ...
    if (this.value && !this.timeOnly) {
      setTimeout(_ => {
        const val = moment(this.value, this.format)
        let dateMeta = {
          day: val.jDate(),
          month: val.jMonth(),
          year: val.jYear(),
  ...
  parseDate(value: any, format: string) {
    ...
    if (
      date.locale('en').year() !== year ||
      date.locale('en').month() + 1 !== month ||
      date.locale('en').date() !== day
    ) {
  ...
  jumpDaySubmit(count) {
    if (count > 0) {
      ...
      if (!this.value) this.value = moment()
      this.updateModel(
        moment(this.value, this.format).subtract(Math.abs(count), 'day')
      )
    } else {
...
@NgModule({
  imports: [CommonModule, ButtonModule, SharedModule, RippleModule, ChevronLeftIcon, ChevronRightIcon, ChevronUpIcon, ChevronDownIcon, TimesIcon, CalendarIcon, OverlayPanelModule, TranslateModule, PrimeCalendar],
  exports: [PrimeCalendar, ButtonModule, SharedModule],
  declarations: []
})
