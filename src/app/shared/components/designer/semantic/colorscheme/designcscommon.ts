...
import { palette, Theme } from '@primeng/themes'
    ...
    template: ` <p-fieldset legend="Common" [toggleable]="true">
        <section class="flex justify-between items-center mb-4">
            <div class="flex gap-2 items-center">
                ...
                <input [value]="themeService.config?.colors?.[dark ? 'surface_dark' : 'surface_light']?.['500']" (input)="onSurfaceColorChange($event)" [type]="'color'" />
            </div>
            <design-color-palette [value]="themeService.config?.colors?.[dark ? 'surface_dark' : 'surface_light']" />
        </section>
...
export class DesignCSCommon {
  ...
  public themeService = inject(ThemeService);
    ...
    onSurfaceColorChange(event) {
      ...
      for (const [wight, val] of Object.entries(this.colorScheme.surface)) {
        document.documentElement.style.setProperty(`--color-surface-${this.dark ? 'dark' : 'light'}-${wight}`, `${val}`);
      }
      config.colors[this.dark ? 'surface_dark' : 'surface_light'] = this.colorScheme.surface;
